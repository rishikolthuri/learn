...
public class UserDAO {
	public User getUser(int id) throws ClassNotFoundException, SQLException{
		...
		Role role = new Role();
		try {
			PreparedStatement ps = conn.prepareStatement("Select USER_ID, NAME, AGE, GENDER, CONTACT_NUMBER, PASSWORD, ROLE_ID from USER_TABLE WHERE User_Id = ?");
			ps.setInt(1,  id);
	...
	public List<User> getAllUsers() throws ClassNotFoundException, SQLException{
		...
		Role role = new Role();
		try {
			PreparedStatement ps = conn.prepareStatement("Select USER_ID, NAME, AGE, GENDER, CONTACT_NUMBER, PASSWORD, ROLE_ID from USER_TABLE");
			ResultSet rs = ps.executeQuery();
			while(rs.next()) {
				User.setName(rs.getString(2));
				User.setAge(rs.getInt(3));
